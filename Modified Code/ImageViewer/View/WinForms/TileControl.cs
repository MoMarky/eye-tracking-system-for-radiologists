...
namespace ClearCanvas.ImageViewer.View.WinForms
{
    ...
    //    public void wirte_line(string line)
    //    {
    ...
    //    public string set_save_path()
    //    {
    //        return "";
    //    }

    //    public void open_csv_writer(string now_time_file_name)
    //    {
    //        csv_writer = new StreamWriter(now_time_file_name, true, System.Text.Encoding.UTF8);
    ...
    public partial class TileControl : UserControl
	{
        ...
        //string now_time_file_name = ".\\save_log\\" + DateTime.Now.ToString("yyyy-MM-dd") +
        //        "_" + DateTime.Now.Hour.ToString() + "-" +
        //        DateTime.Now.Minute.ToString() + "-" +
        //        DateTime.Now.Second.ToString() + "_Pacs.csv";

        //ClearCanvas.ImageViewer.View.WinForms.save_csv save_file;
        ...
        public TileControl(Tile tile, Rectangle parentRectangle, int parentImageBoxInsetWidth)
		{
			...
			InitializeComponent();
            //save_file = new ClearCanvas.ImageViewer.View.WinForms.save_csv();
		...
		private void DoDraw()
		{
            ...
            //Console.WriteLine("<<,<<WPresent...Drawing()>>>>");
            int NowSelectedBoxIndex = -1;
            string nowtime = "";
            ...
            if (this.Surface != null)
			{
                ...
                ///this.Tile.PresentationImage.ParentDisplaySet.PresentationImages.
                ...
                try
				{
                    ...
                    //if (ClearCanvas.Common.PublicMethod._tileDraw_flag == false)
                    //{
                    //    ClearCanvas.Common.PublicMethod._tileDraw_flag = true;
                    //   FPresentDraw_flag = false;
                    _tile.Draw(args);
                    //}
                    //this.str = "SpatialTransform...SomeThing..." + args.SceneGraph.SpatialTransform.ScaleX.ToString()
                    //    + " # " + args.SceneGraph.SpatialTransform.ScaleY.ToString();
                    /*this.str = "ClientRectangle : Height:" + this.ClientRectangle.Height.ToString()
                        + " # Width: " + this.ClientRectangle.Width.ToString()
                        + " # CX: " + this.ClientRectangle.X.ToString()
                        + " # CY : " + this.ClientRectangle.Y.ToString()
                        + " # Bounds: X:" + this.Bounds.X.ToString()
                        + " # Bounds: Y:" + this.Bounds.Y.ToString()
                        + " # Bounds: W:" + this.Bounds.Width.ToString()
                        + " # Bounds: H:" + this.Bounds.Height.ToString()
                        ;*/
                    //long times = (DateTime.UtcNow.Ticks - (new DateTime(1970, 1, 1)).ToUniversalTime().Ticks) / 100;
                    //nowtime = times.ToString();
                    TimeSpan ts = DateTime.UtcNow - new DateTime(1970, 1, 1, 0, 0, 0, 0);
                    nowtime = Convert.ToInt64(ts.TotalMilliseconds*100).ToString();

                    Point Screen_mid = new Point((args.Screen.Bounds.Width - args.Screen.Bounds.X) / 2, (args.Screen.Bounds.Height - args.Screen.Bounds.Y) / 2);

                    if (this.AccessibilityObject.Bounds.X < Screen_mid.X && this.AccessibilityObject.Bounds.Y < Screen_mid.Y)
                            NowSelectedBoxIndex = 0;
                    else if(this.AccessibilityObject.Bounds.X > Screen_mid.X && this.AccessibilityObject.Bounds.Y < Screen_mid.Y)
                            NowSelectedBoxIndex = 1;
                    else if (this.AccessibilityObject.Bounds.X < Screen_mid.X && this.AccessibilityObject.Bounds.Y > Screen_mid.Y)
                            NowSelectedBoxIndex = 2;
                    else if (this.AccessibilityObject.Bounds.X > Screen_mid.X && this.AccessibilityObject.Bounds.Y > Screen_mid.Y)
                            NowSelectedBoxIndex = 3;

                    //ClearCanvas.Common.PublicMethod.SetPresentImageBox(this.Bounds.X, this.Bounds.Y,
                    //    this.Bounds.Width, this.Bounds.Height);
                    ClearCanvas.Common.PublicMethod.SetPresentImageBox_info(this.AccessibilityObject.Bounds.X, this.AccessibilityObject.Bounds.Y,
                        this.AccessibilityObject.Bounds.Width, this.AccessibilityObject.Bounds.Height, NowSelectedBoxIndex);
                    ClearCanvas.Common.PublicMethod.SetScreenPara(args.Screen.Bounds.X, args.Screen.Bounds.Y,
                        args.Screen.Bounds.Width, args.Screen.Bounds.Height);
                    //ClearCanvas.Common.PublicMethod.SetSelectedBoxIndex(NowSelectedBoxIndex);
                    //Console.WriteLine(ClearCanvas.Common.PublicMethod.sp_count.ToString());
                    //ClearCanvas.Common.PublicMethod.sp_count = 0;
                    //ClearCanvas.Common.PublicMethod.SetPresentIm       ageOffset(args.SceneGraph.SpatialTransform.Transform.OffsetX, args.SceneGraph.SpatialTransform.Transform.OffsetY);
                    //args.SceneGraph.ParentPresentationImage.
            ...
            float[] now_iamge_pos = new float[20];
            now_iamge_pos = ClearCanvas.Common.PublicMethod.GetPresentImagePos();

            //string output = " ImageX: " + now_iamge_pos[0].ToString() +
            ...
            //            " Screen_x: " + now_iamge_pos[13].ToString() +
            //            " Screen_y: " + now_iamge_pos[14].ToString() +
            //            " Screen_w: " + now_iamge_pos[15].ToString() +
            //            " Screen_h: " + now_iamge_pos[16].ToString();

            //Console.WriteLine("##" + output + "##" + ClearCanvas.Common.PublicMethod.GetPresentDcmPath());

            //save_file.wirte_line(output);
            ClearCanvas.Common.PublicSaveLog.wirte_array(nowtime, now_iamge_pos, ClearCanvas.Common.PublicMethod.GetPresentDcmPath());
            //save_file.open_csv_writer
            //sw.Write(output + "##" + ClearCanvas.Common.PublicMethod.GetPresentDcmPath());
            //this._tile.ParentImageBox.TopLeftPresentationImage.
